...
TEST_CASE("Production catalogue grid lays out compact two-row pages deterministically", "[production][catalogue][layout]") {
...
TEST_CASE("Production catalogue grid exposes full-row scrolling limits", "[production][catalogue][layout][scroll]") {
    ...
    REQUIRE(eighteen.maxScrollRow == 1);
    ...
    REQUIRE(twentyFive.visibleRows == 2);
    REQUIRE(twentyFive.maxScrollRow == 3);
    REQUIRE(clampProductionCatalogScrollRow(twentyFive, -3) == 0);
    REQUIRE(clampProductionCatalogScrollRow(twentyFive, 5) == 3);
    REQUIRE(scrollProductionCatalogRows(twentyFive, 3, -1) == 2);
...
TEST_CASE("Production catalogue tabs stay directly above the compact grid", "[production][catalogue][layout][input-routing]") {
    ...
    const auto tabs = calculateProductionCatalogTabBounds(grid, 3);
    const auto control = calculateProductionCatalogControlBounds(grid, 3);

    REQUIRE(tabs.width == grid.width);
    REQUIRE(tabs.y + tabs.height + PRODUCTION_CATALOG_PANEL_MARGIN == grid.y);
    REQUIRE(control.y == tabs.y);
    REQUIRE(control.y + control.height == grid.y + grid.height);
    REQUIRE(getProductionCatalogCategoryTabIndexAtPoint(tabs, 3, tabs.x + 1, tabs.y + 1) == 0);
    REQUIRE(getProductionCatalogCategoryTabIndexAtPoint(tabs, 3, tabs.x + tabs.width / 2, tabs.y + 1) == 1);
    REQUIRE(getProductionCatalogCategoryTabIndexAtPoint(tabs, 3, tabs.x + tabs.width - 1, tabs.y + 1) == 2);
    REQUIRE(getProductionCatalogCategoryTabIndexAtPoint(tabs, 3, tabs.x - 1, tabs.y + 1) == -1);
}
...
TEST_CASE("Production catalogue grid hit-test honors actual entry count and reduced columns", "[production][catalogue][layout][input-routing]") {
    ...
    const ProductionCatalogPanelBounds sevenPanel{5, 412, sevenEntries.panelWidth, sevenEntries.panelHeight};
    ...
    const ProductionCatalogPanelBounds reducedPanel{5, 359, reduced.panelWidth, reduced.panelHeight};
...
TEST_CASE("Production catalogue grid reuses the existing production handler only", "[production][catalogue][input-routing]") {
    ...
    REQUIRE(grid.find("getProductionCatalogEntriesForCategory") != std::string::npos);
    REQUIRE(grid.find("getProductionCatalogCategoryTabIndexAtPoint") != std::string::npos);
